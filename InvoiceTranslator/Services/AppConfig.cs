...
namespace InvoiceTranslator.Services
{
    public static class AppConfig
    {
        ...
        public static bool SaveValues()
        {
            ...
            {
                { "InFolderDefault", InFolderDefault },
                { "OutFolderDefault", OutFolderDefault },
                { "IsOpenOutfile", (IsOpenOutfile ? "1" : "0") }
            };

            SaveAppSettings(dict, out string errorMsg);
        ...
        public static DateTime GetDateTimeValue(string key, IFormatProvider provider = null)
        {
            ...
            if (value != null)
            {
                ...
                if (DateTime.TryParse(value, provider, System.Globalization.DateTimeStyles.None, out DateTime dt)) retVal = dt;
            }
            return retVal;
        }
        public static TimeSpan GetTimeSpanValue(string key)
        {
            ...
            if (value != null)
            {
                if (TimeSpan.TryParse(value, System.Globalization.CultureInfo.InvariantCulture, out TimeSpan ts)) retVal = ts;
            }
            return retVal;
        }
